...
To reproduce figures, you need to first download `nonlinear_ensemble_single_cell_space_v_ori_osi_2d.tar.gz` from https://doi.org/10.5281/zenodo.21987736 and extract its content into this directory. You should get a `runs` directory.
...
These commands were run on a SLURM cluster. If you're not on a SLURM cluster, remove `sbatch -c 8 --mem-per-cpu=8gb --time 01:00:00 --output slurm/plot/%A.out` from the commands.
