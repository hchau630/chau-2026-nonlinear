...
def is_int(x: Tensor):
    return not (x.is_floating_point() or x.is_complex())


def cast(
    func: Callable[[Tensor], Tensor | Sequence[Tensor]], *args: Tensor, **kwargs
) -> Tensor | Sequence[Tensor]:
    # hacky fix for the fact that BaseCirculantTensor currently doesn't support
    # is_floating_point(), is_complex(), and to().
    if any(isinstance(x, BaseCirculantTensor) for x in args):
        return func(*args, **kwargs)

    args_ = [x for x in args if not is_int(x)]
    if len(args_) == 0:
        return func(*args, **kwargs)

    dtype = args_[0].dtype if len(args_) == 1 else reduce(torch.result_type, args_)
    if dtype in {torch.double, torch.cdouble, torch.float, torch.cfloat}:
        return func(*args, **kwargs)

    args = [
        x if is_int(x) else x.to(torch.cfloat if x.is_complex() else torch.float)
        for x in args
    ]
    out = func(*args, **kwargs)
    if not isinstance(out, Tensor):
        return [o.to(dtype.to_complex() if o.is_complex() else dtype) for o in out]
    return out.to(dtype.to_complex() if out.is_complex() else dtype)
...
def take_along_dims(
    ...
    *indices: Tensor,
    ...
    for idx, dim in zip(indices, dims, strict=True):
        ...
        A = cast(torch.take_along_dim, A, idx, dim=dim)
...
def concat(
    ...
    cat_cols = set.intersection(
        *[{k for k, v in df.dtypes.items() if v.name == "category"} for df in dfs_]
    )
    for col in cat_cols:
