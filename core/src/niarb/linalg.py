...
def bmv(A: Tensor, x: Tensor):
    if isinstance(A, BaseCirculantTensor):
        return (A @ x.unsqueeze(-1)).squeeze(-1)

    # torch.matmul is hundreds of times slower than einsum for these shapes,
    # see PyTorch GitHub issue #110858
    return torch.einsum("...ij,...j->...i", A, x)


def bvm(x: Tensor, A: Tensor):
    if isinstance(A, BaseCirculantTensor):
        raise NotImplementedError("bvm is not implemented for CirculantTensor.")

    return torch.einsum("...i,...ij->...j", x, A)
...
def nbmv(A, x, ndim1=1, ndim2=1):
    ...
    if A.ndim < ndim1 + ndim2 or x.ndim < ndim2:
        raise ValueError(
            f"A must be at least {ndim1+ndim2=} dimensional and x must be at least"
            f" {ndim2=} dimensional."
        )
