...
from .base import BaseTensor, _rebuild, tensor_class_factory
...
def n_circulant(x, dim=None):
    ...
    """
    from niarb.utils import normalize_dim  # local import to avoid circular import
...
class BaseCirculantTensor(BaseTensor):
    def __new__(cls, data, **kwargs):
        ...
        if cls.block and data.shape[-2] != data.shape[-1]:
            raise ValueError(
                "Last two dimensions of data must be equal if block is True, but"
                f" {data.shape=}."
            )
...
@implements([torch.t, torch.Tensor.t])
def t(func, cls, types, args=(), kwargs=None):
    if len(args) != 1 or not isinstance(args[0], torch.Tensor):
        raise TypeError("t() requires a single tensor argument.")

    data, cdim = args[0].tensor, args[0].cdim
    for dim in cdim:
        # [c_0, c_1, ..., c_{n-1}] -> [c_0, c_{n-1}, ..., c_1]
        data = data.flip(dim).roll(1, dims=dim)
...
@implements([torch.adjoint, torch.Tensor.adjoint])
def adjoint(func, cls, types, args=(), kwargs=None):
    if len(args) != 1 or not isinstance(args[0], torch.Tensor):
        raise TypeError("t() requires a single tensor argument.")

    data = args[0].tensor.conj()
...
@implements([torch.linalg.eigvals, torch.linalg.eigvalsh])
def eigvals(func, cls, types, args=(), kwargs=None):
    ...
    eigvals = func(data, **(kwargs or {}))

    return eigvals.reshape(*eigvals.shape[:-2], -1)


@implements([torch.linalg.inv])
def inv_op(func, cls, types, args=(), kwargs=None):
    (data,) = args
    if cls.block:
        return super(BaseCirculantTensor, cls).__torch_function__(
            func, types, args=args, kwargs=kwargs
        ...
        return data.tensor.reciprocal().as_subclass(cls)
...
def batched_matrix_op(func, cls, types, args=(), kwargs=None):
    ...
    Args:
        ...
        B (Tensor): BaseCirculantTensor or Tensor with shape (*, *A.cshape, *A.bshape)
          or (*, *A.cshape, *A.bshape, k).

    Returns:
        BaseCirculantTensor or Tensor with shape (*, *A.cshape, *A.bshape) or
        (*, *A.cshape, *A.bshape, k).
    ...
    if isinstance(B, BaseCirculantTensor):
        raise TypeError(
            "If B is a BaseCirculantTensor, it must be the same type as A, but "
            f"{type(A)=} and {type(B)=}."
        )
