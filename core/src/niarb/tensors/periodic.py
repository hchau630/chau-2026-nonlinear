...
class PeriodicTensor(BaseTensor):
    def __init__(self, *args, **kwargs):
        ...
        if not all(isinstance(v, int) and 0 <= v < D for v in w_dims):
            raise ValueError(
                f"w_dims must be a sequence of non-negative integers less than {D=},"
                f" but {w_dims=}."
            )

        if len(extents) != len(w_dims):
            raise ValueError(
                f"extents must be a sequence with length {len(w_dims)=}, but"
                f" {len(extents)=}."
            )
    ...
    def low(self):
        return torch.tensor(
            [extent[0] for extent in self.extents],
            device=self.device,
            dtype=self.dtype,
        )
    ...
    def high(self):
        return torch.tensor(
            [extent[1] for extent in self.extents],
            device=self.device,
            dtype=self.dtype,
        )
    ...
    def cvar(
        ...
        **kwargs,
        ...
        if (self.ndim - 1) in dim or -1 in dim:
            raise ValueError(
                "Computing variance across the last dimension is not allowed, but"
                f" {dim=}."
            )
