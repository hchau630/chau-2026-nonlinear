...
from niarb import perturbation, random, special, utils
...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # window: float | Sequence[float] | dict[str, Sequence[float]] = torch.inf,
        abs_window: float | Sequence[float] | dict[str, Sequence[float]] = torch.inf,
        ...
        **forward_kwargs,
        ...
        for k, v in metrics.items():
            kwargs = {}
            if not callable(v):
                if v not in {"min_distance_to_ensemble", "radius", "abs_ori"}:
                    kwargs["keepdim"] = True
    ...
    def __getitem__(self, index):
        ...
        x["mask"] = torch.ones(x.shape, device=x.device, dtype=torch.bool)

        if "space" in x:
            for attr in ["abs_window"]:
                window = getattr(self, attr)
                loc = x["space"]

                if isinstance(window, dict):
                    for k, v in window.items():
                        x["mask"] &= (x["cell_type"] != k) | special.uniform(
                            v, loc
                        ).prod(dim=-1).bool()
                elif window != torch.inf:
                    x["mask"] &= special.uniform(window, loc).prod(dim=-1).bool()

        if self.subsample is not None:
            with random.set_seed(seed):
                x["mask"] = perturbation.categorical_sample(
                    x["mask"].float(), self.subsample
                ).bool()
        ...
        # if "space" in x:
        #     for attr in ["window", "abs_window"]:
        #         window = getattr(self, attr)

        #         if attr == "window" and window != torch.inf:

        #             def func(xi):
        #                 cmean = xi["space"][xi["dh"] != 0].cmean()  # (D,)
        #                 return cmean[(None,) * xi.ndim]  # (1, ..., 1, D)

        #             loc = F.diff(x["space"], x.apply(func, dim=range(1, x.ndim)))
        #         else:
        #             loc = x["space"]
        ...
        #                 x["mask"] &= (
        #                     (x["cell_type"] != k)
        #                     | special.uniform(v, loc).prod(dim=-1).bool()
        #                 )
        #         elif window != torch.inf:
        #             x["mask"] &= special.uniform(window, loc).prod(dim=-1).bool()
        ...
        x["stim_idx"] = torch.arange(x.shape[0])[(...,) + (None,) * (x.ndim - 1)]
