...
def newton(
    ...
    """Use Newton's method to find the root of a scalar function.

    Args:
        func: The scalar function whose first argument's root is solved. Dependence
        ...
        args (optional): Additional tensors to pass to `func`. The leading dimensions of
          each tensor must be broadcastable with `x`. These tensors are vmapped over.
        kwargs (optional): Optional arguments passed to `func`. Note that these
        ...
        tol (optional): Terminate when (func(x, *args, **kwargs).abs() < tol).all(). If
          None, defaults to 1e-6 for float32 and 1e-8 for float64.
    ...
    """
    if maxiter < 0:
        raise ValueError(f"maxiter must be non-negative, but {maxiter=}.")

    if bounds[0] is not None and (x < bounds[0]).any():
        raise ValueError("x must be greater than or equal to the lower bound.")

    if bounds[1] is not None and (x > bounds[1]).any():
        raise ValueError("x must be less than or equal to the upper bound.")

    if tol is None:
        tol = 1e-8 if x.dtype is torch.double else 1e-5

    for n in range(maxiter + 1):
        y = func(x, *args, **(kwargs or {}))
        logger.debug(
            f"{n=}: {y.abs().max().item()=}, {x.min().item()=}, {x.max().item()=}, "
            f"{x.reshape(-1)[y.abs().argmax()].item()=}"
        )
        if (y.abs() < tol).all():
            break

        gradient = numerics.compute_nth_deriv(func, x, args=args, kwargs=kwargs)
        x = x - y / gradient
        if bounds != (None, None):
            x = x.clip(*bounds)
    else:
        raise RuntimeError(
            f"Failed to converge with {tol=} within {maxiter} steps. "
            f"Final maximum absolute residual is {y.abs().max().item()}."
        )
