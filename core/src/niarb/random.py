...
def log_normal_strict(
    ...
    Same as log_normal, but mean and std must be positive.
...
def log_normal(
    ...
    mean, std = torch.as_tensor(mean), torch.as_tensor(std)
    if (mean > 0).all() and (std > 0).all():
        return log_normal_strict(mean, std, validate_args=False, size=size)

    mean, std = torch.broadcast_tensors(mean, std)
    mean, std = mean.broadcast_to(size + mean.shape), std.broadcast_to(size + std.shape)
...
def resample_with_min_dist(
    x: Tensor, sampler: Distribution | Callable[[Tensor], Tensor], min_dist: float
) -> Tensor:
    if min_dist <= 0:
        return x
    ...
    while len(indices) > 0:
        ...
        x[indices] = (
            sampler.sample(indices.shape)
            if isinstance(sampler, Distribution)
