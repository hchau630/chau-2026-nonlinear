...
def get_join_indexers(lkeys, rkeys, backend=None):
    ...
    if backend == "pandas" or (backend is None and lkeys.device.type == "cpu"):
        ...
        lidx, ridx = (
            torch.from_numpy(lidx).to(device),
            torch.from_numpy(ridx).to(device),
        )
...
def get_neighbors(
    ...
    if validate:
        ...
        if lbin.is_floating_point() or rbin.is_floating_point():
            raise TypeError(
                f"lbin and rbin must be integer tensor, but {lbin.dtype=},"
                f" {rbin.dtype=}."
            )
        ...
        if lbin.ndim != 2 or rbin.ndim != 2:
            raise ValueError(
                f"lbin and rbin must be a 2-dimensional tensor, but {lbin.ndim=},"
                f" {rbin.ndim=}."
            )

        if lid.ndim != 1 or rid.ndim != 1:
            raise ValueError(
                f"lid and rid must be a 1-dimensional tensor, but {lid.ndim=},"
                f" {rid.ndim=}."
            )
        ...
        if (lbin.numel() > 0 and (lbin.max(dim=1).values >= nbins).any()) or (
            ...
            raise ValueError(
                f"lbin and rbin must be less than nbins, but {lbin.max(dim=1).values=},"
                f" {rbin.max(dim=1).values}, {nbins=}."
            )
    ...
    if isinstance(x, periodic.PeriodicTensor) and len(x.w_dims) > 0:
        if len(x.w_dims) != x.D:
            raise NotImplementedError(
                "Currently only handles inputs that are either non-periodic or periodic"
                " in all dimensions."
            )
