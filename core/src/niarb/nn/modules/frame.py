...
class ParameterFrame(ParameterDict):
    ...
    def __setitem__(self, key, value):
        ...
        if len(self._keys) != 0:
            try:
                ...
                raise ValueError(
                    "Unable to set ParameterFrame item due to shape mismatch:"
                    f" {self.shape=}, but {value.shape[:self.ndim]=}."
                ) from err
    ...
    def _values(self) -> Iterable[Tensor]:
        return (self.data[k] for k in self._keys)

    def _items(self) -> Iterable[tuple[str, Tensor]]:
    ...
    def device(self):
        ...
        raise ValueError(
            "ParameterFrame must have at least one tensor and all tensors must be on"
            " the same device."
        )
    ...
    def ndims(self) -> Generator[int, None, None]:
        yield from (v.ndim - self.ndim for v in self._values())

    def shapes(self) -> Generator[tuple[int, ...], None, None]:
    ...
    def take_along_dim(self, indices: Tensor, dim: int | None = None) -> Self:
        out = {}
        for (k, v), ndim in zip(self._items(), self.ndims(), strict=True):
            out[k] = v.take_along_dim(indices[(...,) + (None,) * ndim], dim=dim - ndim)
        return ParameterFrame(out, ndim=self.ndim)
    ...
    def explode(self, column=None, dim=0, to_pandas=False):
        ...
        if dim == 0:
            ...
            if not lib.itertools.isconst(self.shapes()):
                raise ValueError(
                    "Shapes of exploded columns must be equal, but got"
                    f" {list(self.shapes())=}."
                )
        ...
        if to_pandas:
            if not ndim == 1:
                raise ValueError(
                    "to_pandas=True is allowed only when resulting ndim=1, but"
                    f" {ndim=}."
                )
...
def concat(
    ...
    if dim > ndim:
        raise ValueError(
            f"dim must be less than or equal to the number of dimensions {ndim}, but"
            f" {dim=}."
        )
    dim = dim % (ndim + 1)
