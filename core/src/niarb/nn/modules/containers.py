...
__all__ = ["FanOut", "NamedSequential", "ParameterDict"]
...
class NamedSequential(torch.nn.Module):
    ...
    def __init__(self, modules: dict[str, torch.nn.Module]):
        ...
        self._allowed_keywords = {f"{name}_kwargs" for name in modules}

    def forward(self, input, **kwargs):
        if any(k not in self._allowed_keywords for k in kwargs):
            raise ValueError(
