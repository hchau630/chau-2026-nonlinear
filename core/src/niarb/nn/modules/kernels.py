...
class Kernel(Function):
    ...
    def validate(self, x: ParameterFrame, y: ParameterFrame):
        ...
        try:
            ...
            raise ValueError(
                f"x and y must have broadcastable shapes, but {x.shape=} and"
                f" {y.shape=}."
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if normalize not in {"none", "integral", "origin"}:
            raise ValueError(
                "normalize must be 'none', 'integral', or 'origin', but got"
                f" {normalize}."
            )
...
class Cosine(Kernel):
    ...
    def __init__(self, kappa: Function, *args, **kwargs):
...
class Tuning(Kernel):
    ...
    def __init__(
        self, kappa: Function, *args, dim: int | Sequence[int] | None = 0, **kwargs
    ):
        super().__init__(*args, kernels=(kappa,), **kwargs)
        self.dim = dim
    ...
    def kernel(self, x: PeriodicTensor, y: PeriodicTensor, kappa: Tensor) -> Tensor:
        theta = diff(x, y)
        return ft.torus.irftn(kappa, theta, dim=self.dim, period=theta.period.tolist())


class VonMises(Kernel):
    n = 1
    ...
    def kernel(self, x: PeriodicTensor, y: PeriodicTensor, kappa: Tensor) -> Tensor:
        theta = diff(x, y)
        out = torch.exp(kappa * torch.cos(theta.to_period(2 * torch.pi).norm(dim=-1)))
        return out / (torch.special.modified_bessel_i0(kappa) * theta.period.prod())


class Scaled(Kernel):
    n = 1

    def __init__(self, kernel: Function, scale: Function, *args, **kwargs):
        super().__init__(*args, kernels=(kernel, scale), **kwargs)

    def kernel(
        self, x: PeriodicTensor, _: PeriodicTensor, kernel: Tensor, scale: Tensor
    ) -> Tensor:
        period = x.period.prod()
        return (kernel - 1 / period) * scale + 1 / period
