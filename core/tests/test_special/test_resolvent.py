...
from niarb import random, special
...
@pytest.mark.parametrize("dr", [0.0, 0.1, [0.1]])
def test_laplace_r(d, is_double, l, dr):
    dr_, dr = (torch.tensor(dr), dr[0]) if isinstance(dr, list) else (dr, dr)
    ...
    if d == 0:
        ...
        if isinstance(s, np.ndarray):
            ...
            integral = np.array(
                [
                    quad(func, 0, dr, args=(si.item(),), complex_func=is_complex)[0]
                    for si in s
                ]
            )
    ...
    with pytest.raises(ValueError) if d <= 1 and dr != 0.0 else nullcontext():
        y_torch = laplace_r(d, l, x_torch, dr=dr_)
...
@pytest.mark.parametrize("l_requires_grad", [False, True])
@pytest.mark.parametrize("dr_requires_grad", [False, True])
def test_laplace_r_grad(d, l, x_requires_grad, l_requires_grad, dr_requires_grad):
    if not any([x_requires_grad, l_requires_grad, dr_requires_grad]):
        pytest.skip("No gradients requested")

    if dr_requires_grad and d <= 1:
        pytest.skip("dr requires grad only for d > 1")
    ...
    dr = (
        0.0
        if d <= 1
        else torch.tensor(0.1, dtype=torch.double, requires_grad=dr_requires_grad)
    )
    ...
    torch.autograd.gradcheck(laplace_r, (d, l, x, dr))
