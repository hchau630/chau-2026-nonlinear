...
def test_matrix_op(op, block, nc, is_mat, batched_c, batched_B, dtype):
    ...
    torch.testing.assert_close(out, expected, rtol=1.3e-5, atol=1.5e-5)
...
def test_matrix_op_circulant(op, block, nc, batched_A, batched_B, complex):
    if nc == 0 and block is False:
        pytest.skip()
    ...
    shape = tuple(3 * i for i in range(1, nc + 1))
    block = (2, 2) if block else ()
    batch_A = (4,) if batched_A else ()
    batch_B = (4,) if batched_B else ()

    with random.set_seed(0):
        A = torch.randn((*batch_A, *shape, *block), dtype=torch.double)
        B = torch.randn((*batch_B, *shape, *block), dtype=torch.double)
        if complex:
            A = A + 1.0j
            B = B + 2.0j

    A = base_circulant.as_tensor(A, nc=nc, block=bool(block))
    B = base_circulant.as_tensor(B, nc=nc, block=bool(block))
...
@pytest.mark.parametrize("complex", [True, False])
@pytest.mark.parametrize("func", ["eigvals", "eigvalsh"])
def test_eigvals(block, nc, batched, complex, func):
    if nc == 0 and block is False:
        pytest.skip()
    ...
    with random.set_seed(0):
        c = torch.randn((*batch, *shape, *block_c), dtype=torch.double)
        if complex:
            c = c + 1.0j
    ...
    out = func(A)
    expected = func(A.dense(keep_shape=False))

    torch.testing.assert_close(out.real.sort()[0], expected.real.sort()[0])
    if torch.is_complex(out):
        torch.testing.assert_close(out.imag.sort()[0], expected.imag.sort()[0])
...
@pytest.mark.parametrize("batched", [True, False])
@pytest.mark.parametrize("complex", [True, False])
def test_t(block, nc, batched, complex):
    if nc == 0 and block is False:
        pytest.skip()
    ...
    out = A.t().dense(keep_shape=False)
    expected = A.dense(keep_shape=False).mT
...
@pytest.mark.parametrize("batched", [True, False])
@pytest.mark.parametrize("complex", [True, False])
def test_adjoint(block, nc, batched, complex):
    if nc == 0 and block is False:
        pytest.skip()
    ...
    out = A.adjoint().dense(keep_shape=False)
    expected = A.dense(keep_shape=False).adjoint()
...
@pytest.mark.parametrize("batched", [True, False])
@pytest.mark.parametrize("complex", [True, False])
def test_inv(block, nc, batched, complex):
    if nc == 0 and block is False:
        pytest.skip()
    ...
    out = torch.linalg.inv(A).dense(keep_shape=False)
    expected = torch.linalg.inv(A.dense(keep_shape=False))
