    ...
    return frame.ParameterFrame(
        {
            ...
            "osi": torch.tensor([0.5, 1.0, 1.0, 0.5]),
    ...
    return frame.ParameterFrame(
        {
            ...
            "osi": torch.tensor([1.0, 1.0, 0.5, 0.0]),
...
def test_cosine(x, y):
    ...
    kernel = nn.Cosine(nn.Matrix(kappa, "cell_type"), "ori")
    out = kernel(x, y)
    # Δ ori = [45, 90, 0, 45], cos(Δ ori) = [0, -1, 1, 0]
    # 2 * kappa = [1, 3, 4, 2]
    expected = torch.tensor([1.0, -2.0, 5.0, 1.0]) / 180
    torch.testing.assert_close(out, expected)


def test_tuning(x, y):
    kappa = torch.tensor([[1.0, 2.0], [3.0, 4.0]]) / 2
    kappa = torch.stack([torch.ones_like(kappa), kappa])
    kernel = nn.Tuning(nn.Matrix(kappa, "cell_type"), "ori")
    out = kernel(x, y)
    # Δ ori = [45, 90, 0, 45], cos(Δ ori) = [0, -1, 1, 0]
    # 2 * kappa = [1, 3, 4, 2]
    expected = torch.tensor([1.0, -2.0, 5.0, 1.0]) / 180.0
    torch.testing.assert_close(out, expected)


def test_von_mises(x, y):
    kappa = torch.tensor([[1.0, -2.0], [3.0, -4.0]])
    kernel = nn.VonMises(nn.Matrix(kappa, "cell_type"), "ori")
    out = kernel(x, y)
    # Δ ori = [45, 90, 0, 45], kappa = [1, 3, -4, -2]
    expected = torch.tensor([0.125708, 0.00162348, 0.000257923, 0.0698175])
    expected = expected * torch.pi / 90.0
    torch.testing.assert_close(out, expected)
...
def test_scaled(x, y):
    kappa = torch.tensor([[1.0, 2.0], [3.0, 4.0]]) / 2
    kappa = torch.stack([torch.ones_like(kappa), kappa])
    ori_kernel = nn.Tuning(nn.Matrix(kappa, "cell_type"), "ori")
    scale_kernel = nn.RankOne(lambda x: x, x_keys="osi")
    kernel = nn.Scaled(ori_kernel, scale_kernel, "ori")
    out = kernel(x, y)
    # Δ ori = [45, 90, 0, 45], cos(Δ ori) = [0, -1, 1, 0]
    # 2 * kappa * f(mu) * g(nu) = [0.5, 3, 2, 0]
    expected = torch.tensor([1.0, -2.0, 3.0, 1.0]) / 180.0
    torch.testing.assert_close(out, expected)
