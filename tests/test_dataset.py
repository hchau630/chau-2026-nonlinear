...
from niarb import dataset, neurons, perturbation, random
...
class TestDataset:
    ...
    def grid_dset_kwargs(self):
        return {
            ...
            "perturbations": {"configs": [{"N": 1, "repeats": 3}]},
    ...
    def test_getitem(self, dset_kwargs, psf):
        ...
        assert x.data["mask"].shape == (1, 500)
        assert all(
            x.data[k].shape == (4, 500) for k in ("dh", "min_distance_to_ensemble")
        )
        # assert all(
    ...
    @pytest.mark.parametrize("window_cell_types", [["PYR", "PV"], ["PYR"]])
    def test_getitem_window_dict(self, dset_kwargs, window_cell_types):
        dset_kwargs = dset_kwargs.copy()
        dset_kwargs["neurons"] = {
            "N": 1000,
        ...
        abs_window = {"PYR": 800.0, "PV": 500.0}
        dset_kwargs["abs_window"] = {k: abs_window[k] for k in window_cell_types}
        dset = dataset.Dataset(**dset_kwargs)
        x, _, _ = dset[0]
        for k in window_cell_types:
            v = abs_window[k] / 2
            assert (x["space"][(x["cell_type"] == k) & x["mask"]].abs() <= v).all()

        for k in ["PYR", "PV"]:
            if k == "PV" and k in window_cell_types:
                continue
            assert ~(x["space"][(x["cell_type"] == k) & x["mask"]].abs() <= 250.0).all()
