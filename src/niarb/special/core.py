...
def beta(
    ...
    alpha = torch.as_tensor(alpha, dtype=x.dtype, device=x.device)
    beta = torch.as_tensor(beta, dtype=x.dtype, device=x.device)
    return torch.distributions.Beta(alpha, beta).log_prob(x).exp()
...
class ModifiedBesselK1(torch.autograd.Function):
    @staticmethod
    def forward(z):
        return torch_bessel.ops.modified_bessel_k1(z)
...
def k1(z: Tensor) -> Tensor:
    ...
    dtype = None
    if z.dtype not in {torch.float, torch.cfloat, torch.double, torch.cdouble}:
        dtype = z.dtype
        z = z.to(torch.cfloat) if z.is_complex() else z.to(torch.float)
    out = ModifiedBesselK1.apply(z)
    if dtype:
...
class ModifiedBesselI(torch.autograd.Function):
    @staticmethod
    def forward(v, z):
        return torch.from_numpy(scipy_special.iv(v, z.cpu().numpy())).to(z.device)
    ...
    def backward(ctx, grad_output):
        ...
        (z,) = ctx.saved_tensors
        grad = (iv(ctx.v - 1, z) + iv(ctx.v + 1, z)).conj() / 2

        return None, grad.mul_(grad_output)


def iv(v: float, z: Tensor) -> Tensor:
    """Modified Bessel function of the first kind of order v.

    Args:
        v: Order of the Bessel function.
        z: Input tensor with shape (*).

    Returns:
        Tensor with shape (*).

    """
    return ModifiedBesselI.apply(v, z)
