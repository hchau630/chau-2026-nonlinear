...
def irftn(
    ...
    norm: str = "backward",
    period: float | Iterable[float] = 2 * torch.pi,
) -> Tensor:
    r"""
    ...
    Args:
        a: Tensor with shape (*). Fourier coefficients of a real function on the d-torus.
        x: Tensor with shape (**, d).  Locations at which to evaluate the output function.
        dim (optional): Dimensions along which to perform the inverse real Fourier transform.
            If None, all dimensions are used. Assumes that only the positive frequencies are provided
            along the last dim.
        norm (optional): One of {'forward', 'ortho', 'backward'}.
        ...
        period (optional): Period along each dimension. If an Iterable,
            then the number of elements must match number of dimensions.
    Returns:
        out: Tensor with shape broadcast((s for s in a.shape if s not in dim), **)
    """
    if dim is None:
        dim = range(a.ndim)
...
def irft(
    ...
    Args:
        a: Tensor with shape (*). Fourier coefficients of a real function on the 1-torus.
        x: Tensor with shape (**).  Locations at which to evaluate the output function.
        dim (optional): Dimension along which to perform the inverse real Fourier transform.
        norm (optional): One of {'forward', 'ortho', 'backward'}.
            If norm == 'backward', scale by 1 / period.
            If norm == 'ortho', scale by 1 / period ** 0.5.
            If norm == 'forward', no scaling.
        period (optional): Period of the 1-torus.
    Returns:
        out: Tensor with shape broadcast((s for s in a.shape if s != dim), **)
    """
...
def ift(
    ...
    Args:
        a: Tensor with shape (*). Fourier coefficients of a complex function on the 1-torus.
        x: Tensor with shape (**).  Locations at which to evaluate the output function.
        dim (optional): Dimension along which to perform the inverse Fourier transform.
        norm (optional): One of {'forward', 'ortho', 'backward'}.
            If norm == 'backward', scale by 1 / period.
            If norm == 'ortho', scale by 1 / period ** 0.5.
            If norm == 'forward', no scaling.
