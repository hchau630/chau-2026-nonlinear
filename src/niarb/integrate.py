...
def odeint(
    ...
    if "options" not in kwargs or (
        ...
        kwargs["options"] = {"max_num_steps": 100}
    ...
    if event_fn is None:
        ...
        if t.ndim != 0:
            raise ValueError(
                "t must be a zero dimensional tensor when event_fn is None, but"
                f" {t.shape=}."
            )
    ...
    try:
        ...
        if str(err).startswith("max_num_steps exceeded"):
            raise exceptions.SimulationError(
                "Simulation steps exceeded max_num_steps. Try increasing"
...
def odeint_ss(
    ...
    **kwargs,
    ...
    Args:
        ...
        max_x: maximum absolute value of x allowed before termination
    ...
    has_exceeded_max_x, has_exceeded_max_t, has_exceeded_max_dxdt = False, False, False

    def event_fn(
        t, x, *args, dx_rtol=dx_rtol, dx_atol=dx_atol, max_t=max_t, max_dxdt=max_dxdt
    ):
        nonlocal has_exceeded_max_x, has_exceeded_max_t, has_exceeded_max_dxdt

        dxdt = func(t, x, *args)
        allclose = (dxdt.abs() <= x.abs() * dx_rtol + dx_atol).all()
        exceeded_max_x = (x.abs() > max_x).any()
        exceeded_max_t = t > max_t
        exceeded_max_dxdt = (dxdt.abs() > max_dxdt).any()
        result = (
            1.0
            - (allclose | exceeded_max_x | exceeded_max_t | exceeded_max_dxdt).float()
        ).unsqueeze(0)
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            logger.debug(
                f"{dxdt.mean().item()}, {x.mean().item()}, {x.max().item()},"
                f" {x.min().item()}, {t.item()}"
            )

        has_exceeded_max_x |= exceeded_max_x
    ...
    if assert_convergence and has_exceeded_max_t:
        raise exceptions.SimulationError(
            f"Failed to converge to steady state with {dx_rtol=}, {dx_atol=} within"
            f" maximum time {max_t}."
        )

    if has_exceeded_max_x:
        raise exceptions.SimulationError(f"x exceeded maximum allowed value {max_x}.")
