...
def radius(x: frame.ParameterFrame, variable: str = "space") -> torch.Tensor:
    return x[variable].norm(dim=-1)


def abs_ori(x: frame.ParameterFrame, variable: str = "ori") -> torch.Tensor:
    return x[variable].norm(dim=-1)
...
def inter_target_distance_statistics(
    ...
    return frame.ParameterFrame(out, ndim=(x.ndim if keepdim else 0))
...
def signed_relative_ori(
    x: frame.ParameterFrame, perturbation_var: str = "dh", keepdim: bool = False
) -> periodic.PeriodicTensor:
    return F.diff(x["ori"], ensemble_ori(x, perturbation_var, keepdim)).tensor.squeeze(
        -1
    )
...
def categorical_sample(
    ...
    if N > count:
        raise ValueError(
            "Number of targets must not exceed number of non-zero probabilities, but"
            f" got {N=}, {prob.count_nonzero()=}."
        )  # Note: PyTorch says multinomial will handle this error, but there is a bug such that it doesn't
    elif N > count / 2:
        warnings.warn(
            "Number of targets more than half the number of non-zero probabilities,"
...
def perturbation_prob_eval(
    ...
    **kwargs,
    ...
    Args:
        x: ParameterFrame with possible columns ['cell_type', 'space', 'ori', 'osi'].
        expr: Expression to evaluate for each neuron, using the columns of x.
        cell_probs: Dictionary of relative probabilities of each cell type.
          If a cell type is missing from the dictionary, it is assumed to have 0 probability.
          If None, assume uniform distribution.
        space_loc: Mean of spatial location.
    ...
    if "special" in kwargs or "torch" in kwargs:
        raise ValueError("'special' and 'torch' cannot be in kwargs.")
    kwargs["special"] = special
    kwargs["torch"] = torch

    if "space" in x:
        if "space" in kwargs or "dspace" in kwargs:
            raise ValueError(
                "If 'space' is in x, 'space' and 'dspace' cannot be in kwargs."
            )
        kwargs["space"] = x["space"]
        kwargs["dspace"] = F.diff(x["space"], space_loc)

    if "ori" in x:
        if "ori" in kwargs or "dori" in kwargs:
            raise ValueError("If 'ori' is in x, 'ori' and 'dori' cannot be in kwargs.")
        kwargs["ori"] = x["ori"].to_period(2 * torch.pi).tensor.squeeze(-1)
        kwargs["dori"] = (
            F.diff(x["ori"], ori_loc).to_period(2 * torch.pi).tensor.squeeze(-1)
        )

    if "osi" in x:
        if "osi" in kwargs:
            raise ValueError("If 'osi' is in x, 'osi' cannot be in kwargs.")
        kwargs["osi"] = x["osi"]

    if "cell_type" in x and cell_probs is not None:
        v = x["cell_type"]
        cell_probs = [cell_probs.get(ct, 0.0) for ct in v.categories]
        prob *= torch.tensor(cell_probs)[v]
    ...
    if "mask" in x:
        if "mask" in kwargs:
            raise ValueError("If 'mask' is in x, 'mask' cannot be in kwargs.")
        kwargs["mask"] = x["mask"]

    prob *= eval(expr, {}, kwargs)
    logger.debug(f"Number of non-zero prob: {prob.count_nonzero()}/{prob.numel()}")
    return prob
...
def sample(
    ...
    expr: str = "",
    cond: Callable[[frame.ParameterFrame], bool] | str = "",
    **kwargs,
) -> torch.Tensor:
    """Sample a perturbation vector consisting of N perturbed neurons.

    Table of output for different combinations of (N, ignore_value):
    (None, False): N = (prob.sum() / value).round() neurons are perturbed with
        perturbation strength = value.
    (None, True): All neurons are perturbed with perturbation strength = prob.
    (int, False): N neurons are perturbed with perturbation strength = value.
    (int, True): N neurons are perturbed with perturbation strength = prob.sum() / N.

    Args:
        x: ParameterFrame with possible columns ['cell_type', 'space', 'ori', 'osi'].
        N (optional): Number of perturbation targets. If None and not ignore_value, N is
          given by (prob.sum() / value).round(). If None and ignore_value, all neurons
          are perturbed.
        affine_space (optional): If True, sample a random mean spatial location for the
          perturbed ensemble.
        affine_ori (optional): If True, sample a random mean orientation for the
          perturbed ensemble.
        value (optional): Perturbation strength. Ignored if ignore_value is True.
        ignore_value (optional): If True and N is None, value is set as prob. If True
          and N is not None, value is set as prob.sum() / N.
        expr (optional): Expression for the evaluation of prob.
        cond (optional): Filter condition for the sampled neurons, resampling if not met.
          Cannot be provided if N is None and ignore_value is True.
    ...
    if N is None and ignore_value and cond:
        raise ValueError(
            "`cond` cannot be provided when N is None and ignore_value is True."
        )
    ...
    if affine_space:
        ...
        if sorted(space.w_dims) != list(range(space.D)):
            raise ValueError(
                "If affine_space is True, x['space'] must be periodic in all"
                " dimensions."
            )
    ...
    if expr:
        prob = perturbation_prob_eval(
            x, expr, space_loc=space_loc, ori_loc=ori_loc, **kwargs
        )
    else:
        prob = perturbation_prob(x, space_loc=space_loc, ori_loc=ori_loc, **kwargs)

    if N and ignore_value:
        value = prob.sum().item() / N

    if not N and not ignore_value:
        N = round((prob.sum() / value).item())

    out = categorical_sample(prob, N, value=value) if N else prob

    if cond:
        x = x.copy()
        x["dh"] = out
        while not (cond(x) if callable(cond) else eval(cond, None, {"x": x})):
            x["dh"] = categorical_sample(prob, N, value=value)
        out = x["dh"]

    logger.debug(
        f"dh: {N=}, {value=}, mean={out.mean()}, max={out.max()}, min={out.min()}"
    )
    return out
